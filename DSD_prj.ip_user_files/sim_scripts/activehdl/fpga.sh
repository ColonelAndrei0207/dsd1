...
#
# Script generated by Vivado on Tue May 27 21:52:08 +0300 2025
...
#
# Usage        : fpga.sh
#                fpga.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                fpga.sh [-reset_run]
#                fpga.sh [-reset_log]
#                fpga.sh [-help]
#
...
#                script (type 'fpga.sh -help' for more information).
#
...
# script info
echo -e "fpga.sh - Script generated by export_simulation (Vivado v2024.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./fpga.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
map_setup_file()
{
  ...
  if [[ ($lib_map_path == "") ]]; then
    lib_map_path="E:/facultate/MASTER"
  fi
  if [[ ($lib_map_path != "") ]]; then
...
Usage: fpga.sh [-reset_log]\n\
Usage: fpga.sh [-keep_index]\n\
